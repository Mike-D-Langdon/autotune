...
def correct(f0):
    ...
    c_minor_degrees = np.concatenate(
        (c_minor_degrees, [c_minor_degrees[0] + 12]))
...
def correct_pitch(f0):
    ...
    smoothed_corrected_f0[np.isnan(
        smoothed_corrected_f0)] = corrected_f0[np.isnan(smoothed_corrected_f0)]
...
def main():
    y, sr = librosa.load("some_file.wav", sr=None, mono=False)
    ...
    filepath = Path("some_file.wav")
    output_filepath = filepath.parent / \
        (filepath.stem + "_pitch_corrected.wav" + filepath.suffix)
